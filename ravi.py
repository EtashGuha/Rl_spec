...
def sample_tokens(proposed_output, verified_output) -> torch.Tensor:
    ...
    for t in range(proposed_output.generated_len):
        sampled_ratios = (
            verified_output.output_distribution[t, proposed_output.output_ids[0, t]]
        ...
        if rs < sampled_ratios:
            ...
            all_accepted = False
            if (
                verified_output.output_distribution[t, :].shape
...
def target_sample_from_distribution(target_distribution, draft_distribution):
    ...
    distribution = torch.max(distribution, torch.zeros_like(distribution))
            ...
            for _ in range(self.num_rollouts):
                ...
                for i in range(self.max_proposals_per_step):
                    output = self.draft_model(
                        draft_input, output_hidden_states=True, return_dict=True
                    )
                    if len(scores) == 0:
                        ...
                        draft_input = torch.cat(
                            [
                                draft_input,
                                torch.multinomial(
                                    F.softmax(output.logits[0][-1]), num_samples=1
                                ).unsqueeze(dim=0),
                            ],
                            dim=-1,
                        )
                ...
                target_outputs = self.target_model(
                    target_input, output_hidden_states=True, return_dict=True
                )
                target_logits = target_outputs.logits[
                    :, current_input.size(1) - 1 : -1, :
                ]
                target_probs = F.softmax(target_logits, dim=-1)
                target_tokens = torch.multinomial(
                    target_probs.squeeze(0), num_samples=1
                )
                draft_output_and_cache = OutputAndCache(
                    output_ids=proposed_tokens.T,
                    generated_len=len(proposed_tokens),
                    output_logits=draft_logits,
                    output_distribution=draft_probs,
                )
                target_output_and_cache = OutputAndCache(
                    output_ids=target_tokens.T,
                    generated_len=len(target_tokens),
                    output_logits=target_logits,
                    output_distribution=target_probs.squeeze(0),
                )

                accepted, alpha, sample_steps, num_tokens_accepted = sample_tokens(
                    draft_output_and_cache, target_output_and_cache
                )
                probs = []
                ...
                empirical_alpha = total_generated_tokens / (
                    (self.max_proposals_per_step + 1) * propose_steps
                )  # accepted_tokens_per_iter / ((k+1) * num_cycles)
...
class SpeculativePPOTrainer:
    def __init__(
        ...
        self.policy_optimizer = optim.SGD(
            ...
            + list(self.draft_value_head.parameters()),
            lr=config.learning_rate,
        )
        (
            self.draft_model,
            self.target_model,
            self.draft_value_head,
            self.target_value_head,
    ...
    def compute_draft_model_probs(
        ...
        for _ in range(num_rollouts):
            ...
            for i in range(max_proposals_per_step):
                output = draft_model(
                    input_ids.squeeze(0), output_hidden_states=True, return_dict=True
                )
                if len(scores) == 0:
                    scores = output.logits[0][-1].unsqueeze(0)
                else:
            ...
            if len(values) == 0:
                values = draft_value_head(
                    output.hidden_states[-1][-1][-1].float()
                ).unsqueeze(0)
            else:
                values = torch.cat(
                    [
    ...
    def train_step(
        ...
        new_log_probs, new_values = self.compute_draft_model_probs(
            self.draft_model,
            self.draft_value_head,
            input_ids,
            self.num_rollouts,
            self.spec_decoder.max_proposals_per_step,
        )
        ...
        return {
            ...
            "approx_kl": ((old_log_probs - new_log_probs) * torch.exp(old_log_probs))
            .sum(dim=-1)
            .mean()
            .item(),
        }
    ...
    def train(self):
        ...
        for epoch in range(self.config.max_epochs):
            ...
            for batch in tqdm(
                ...
                spec_outputs = [
                    self.spec_decoder.generate(input_ids.to(accelerator.device))
                ...
                _, old_values = self.compute_draft_model_probs(
                    self.draft_model,
                    self.draft_value_head,
                ...
                stats = self.train_step(
                    torch.stack(
                        [output["probability"] for output in spec_outputs]
                    ).cuda(),
                    old_values,
                    advantages,
                    returns,
...
def init_wandb(config: PPOConfig):
    ...
    if accelerator.is_local_main_process:
        wandb.init(project="speculative-ppo", name=run_name, config=config.__dict__)
...
def parse_args():
    ...
    # Model arguments
    parser.add_argument(
        "--draft_model_path", type=str, required=True, help="Path to draft model"
    )
    parser.add_argument(
        "--target_model_path", type=str, required=True, help="Path to target model"
    )
...
def prepare_dataset(dataset, tokenizer, text_column, max_length):
    ...
    def tokenize_function(example):
        ...
        # Tokenize
        tokenized = tokenizer(
            texts, max_length=max_length, truncation=True, return_tensors="pt"
        )
...
def main():
    ...
    target_model = AutoModelForCausalLM.from_pretrained(
        args.target_model_path, torch_dtype=torch.float16
    )
    ...
    processed_dataset = prepare_dataset(
        dataset, tokenizer, args.text_column, args.max_length
    )
